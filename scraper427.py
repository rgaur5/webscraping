...
from sql_query_builder import create_table, add_nonduplicate_row #need these for dynamic SQL queris; functions' precise descriptions can be found in sql_query_builder
...
class scraper427Spider(scrapy.Spider): #necessary formatting to run scraper through terminal (see above)
    ...
    def parse(self, response): #this function gathers all Car data and adds it to a table in a pre-defined database
        db_name = 'Car_Web_Scraping' #eee
        table_name = 'Cars'

        mycursor = mydb.cursor() #creates cursor to access database
